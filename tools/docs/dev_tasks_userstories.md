...
- [ ] **FB-01**: Firebase Facebook認証設定
  - [ ] Facebook Developer アプリ作成
    - [ ] アプリID・シークレット取得
    - [ ] OAuth リダイレクトURL設定
    - [ ] 必要な権限スコープ設定（email, public_profile）
  - [ ] Firebase Auth Facebook プロバイダー設定
    - [ ] Firebase コンソールでFacebook認証有効化
    - [ ] アプリID・シークレット設定
    - [ ] OAuth リダイレクトURL確認
  - [ ] 開発環境でのテスト設定
    - [ ] テストユーザー作成
    - [ ] ローカル開発環境での動作確認
- [ ] **FB-02**: フロントエンド認証フロー実装
  - [ ] ログインビューの更新
    - [ ] Facebook ログインボタン追加
    - [ ] ボタンデザイン・アイコン実装
    - [ ] 既存のGoogle認証と統一的なUI
  - [ ] 認証処理実装（auth.ts store）
    - [ ] signInWithFacebook 関数追加
    - [ ] Facebook OAuth ポップアップ処理
    - [ ] 認証成功/失敗ハンドリング
    - [ ] エラーメッセージの日本語化
  - [ ] 認証状態管理の拡張
    - [ ] プロバイダー情報の保存
    - [ ] 複数プロバイダーのリンク機能
    - [ ] アカウント切り替え対応
- [ ] **FB-03**: バックエンドトークン管理更新
  - [ ] ユーザー作成トリガー関数の更新
    - [ ] Facebook認証ユーザーの初期化処理
    - [ ] プロフィール情報の取得・保存
    - [ ] デフォルトプラン（Free）の設定
  - [ ] トークン検証・リフレッシュ機能
    - [ ] Facebook アクセストークンの検証
    - [ ] 必要に応じたトークンリフレッシュ
    - [ ] セキュリティミドルウェアの更新
  - [ ] ユーザー情報の統合管理
    - [ ] Google/Facebook両認証の統一的な扱い
    - [ ] プロバイダー切り替え時のデータ保持
    - [ ] アカウントマージ機能の検討
- [ ] **FB-04**: テスト・品質保証
  - [ ] 単体テスト作成
