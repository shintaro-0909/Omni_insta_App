...
- [x] **REPO-SETUP**: リポジトリ・CI/CD設定
  - [x] GitHubリポジトリ作成・初回プッシュ
    - [x] リモートリポジトリ作成
    - [x] ローカルから初回プッシュ
    - [x] ブランチ保護設定
  - [ ] GitHub Secrets設定
  ...
  - [x] CI/CDパイプライン動作確認
    - [x] developブランチ → 開発環境デプロイテスト
    - [ ] mainブランチ → 本番環境デプロイテスト
    - [x] 品質ゲートの動作確認

### 3.2 Firebase環境構築
- [x] **FIREBASE-ENV**: 開発・本番環境作成
  - [x] 開発環境作成
    - [x] Firebase プロジェクト作成 (omniy-dev)
    - [x] Cloud Functions デプロイ
    - [x] Hosting デプロイ
    - [x] Firestore設定・インデックス作成
    - [x] 動作確認テスト
  - [ ] 本番環境作成
...
### 4.2 Phase 1拡張機能 (実装予定)

#### **FB-LOGIN**: Facebook認証統合 (Phase 1優先機能)
- [x] **FB-01**: Firebase Facebook認証設定
  - [x] Facebook Developer アプリ作成
    - [x] アプリID・シークレット取得
    - [x] OAuth リダイレクトURL設定
    - [x] 必要な権限スコープ設定（email, public_profile）
  - [x] Firebase Auth Facebook プロバイダー設定
    - [x] Firebase コンソールでFacebook認証有効化
    - [x] アプリID・シークレット設定
    - [x] OAuth リダイレクトURL確認
  - [x] 開発環境でのテスト設定
    - [x] テストユーザー作成
    - [x] ローカル開発環境での動作確認
- [x] **FB-02**: フロントエンド認証フロー実装
  - [x] ログインビューの更新
    - [x] Facebook ログインボタン追加
    - [x] ボタンデザイン・アイコン実装
    - [x] 既存のGoogle認証と統一的なUI
  - [x] 認証処理実装（auth.ts store）
    - [x] signInWithFacebook 関数追加
    - [x] Facebook OAuth ポップアップ処理
    - [x] 認証成功/失敗ハンドリング
    - [x] エラーメッセージの日本語化
  - [x] 認証状態管理の拡張
    - [x] プロバイダー情報の保存
    - [x] 複数プロバイダーのリンク機能
    - [x] アカウント切り替え対応
- [x] **FB-03**: バックエンドトークン管理更新
  - [x] ユーザー作成トリガー関数の更新
    - [x] Facebook認証ユーザーの初期化処理
    - [x] プロフィール情報の取得・保存
    - [x] デフォルトプラン（Free）の設定
  - [x] トークン検証・リフレッシュ機能
    - [x] Facebook アクセストークンの検証
    - [x] 必要に応じたトークンリフレッシュ
    - [x] セキュリティミドルウェアの更新
  - [x] ユーザー情報の統合管理
    - [x] Google/Facebook両認証の統一的な扱い
    - [x] プロバイダー切り替え時のデータ保持
    - [ ] アカウントマージ機能の検討
- [x] **FB-04**: テスト・品質保証
  - [x] 単体テスト作成
...
2. [ ] Firebase本番環境作成・デプロイ
3. [ ] Facebook認証のマルチプロバイダー切り替えテスト実装

### ⭐ **来週 (重要)**
1. [ ] 本番環境でのE2Eテスト最終確認
2. [ ] ユーザー向けドキュメントの最終レビュー
3. [ ] ソフトローンチ準備

### 📋 **今月末 (必要)**
1. [ ] ユーザーからのフィードバック収集と分析
2. [ ] 正式リリースに向けた最終調整
3. [ ] 運用体制の確立
...
### 進行中項目
- **リリース準備**: 8/10 (80%) 🔄
  - GitHub・CI/CD環境: 2/3 (66%) 🔄
  - Firebase環境: 1/2 (50%) 🔄
- **将来拡張機能 (Phase 1)**: 1/1 (100%) ✅
  - Facebook認証統合: 3.5/4 (87%) 🔄

### 目標達成予定
- **MVP リリース**: 2025年2月末 (完了)
- **ソフトローンチ**: 2025年7月上旬
- **正式リリース**: 2025年7月中旬
