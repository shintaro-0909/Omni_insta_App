    ...
    <section class="accounts-section">
      ...
      <div v-if="currentView === 'grid'" class="accounts-grid">
        ...
        >
          <div class="card-header">
            ...
            <div class="account-actions">
              <button class="action-btn" @click.stop="editAccount(account)">⚙️</button>
          ...
          <div class="performance-chart">
            <canvas :ref="`chart-${account.id}`" width="250" height="80"></canvas>
          </div>
        ...
        <div class="add-account-card" @click="showAddAccountModal = true">
          ...
          <p class="add-description">Connect another Instagram account to manage</p>
        </div>
      </div>
      ...
      <div v-if="currentView === 'list'" class="accounts-list">
        ...
        >
          ...
          <div class="row-column followers-col">
            <div class="metric-value">{{ formatNumber(account.followers) }}</div>
          ...
          <div class="row-column growth-col">
            ...
            <div class="growth-trend" :class="account.growthTrend">
              {{ account.growthTrend === 'positive' ? '↗️' : account.growthTrend === 'negative' ? '↘️' : '→' }}
            </div>
          </div>
          ...
          <div class="row-column actions-col">
            <div class="row-actions">
              <button class="action-btn-small" @click="viewAccount(account)">👁️</button>
              <button class="action-btn-small" @click="editAccount(account)">✏️</button>
              <button class="action-btn-small" @click="showAccountMenu(account)">⋯</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-if="selectedAccount" class="modal-overlay" @click="selectedAccount = null">
      <div class="account-modal" @click.stop>
        ...
        <div class="modal-content">
          <div class="modal-stats">
            <div class="modal-stat">
              <div class="stat-number">{{ formatNumber(selectedAccount.followers) }}</div>
              <div class="stat-label">Followers</div>
              <div class="stat-change positive">+{{ selectedAccount.followersChange }}</div>
            </div>
            <div class="modal-stat">
              <div class="stat-number">{{ selectedAccount.following?.toLocaleString() }}</div>
            ...
            <div class="modal-stat">
              ...
              <div class="stat-change positive">+{{ selectedAccount.engagementChange }}%</div>
            </div>
          </div>
            ...
            <div class="recent-posts">
              ...
              >
                ...
                <div class="post-stats">
                  <span class="post-likes">{{ formatNumber(post.likes) }} ❤️</span>
        ...
        <div class="modal-footer">
          <button class="btn-secondary" @click="disconnectAccount(selectedAccount)">
            Disconnect
          </button>
    ...
    <div v-if="showAddAccountModal" class="modal-overlay" @click="showAddAccountModal = false">
      <div class="add-account-modal" @click.stop>
        <div class="modal-header">
          ...
          <button class="modal-close" @click="showAddAccountModal = false">×</button>
        </div>
        ...
        <div class="modal-content">
          <div class="connection-options">
            <div class="connection-method" @click="connectInstagram">
              ...
              <p class="method-description">Connect your Instagram Business account for full features</p>
            ...
            <div class="connection-method" @click="connectCreator">
              ...
              <p class="method-description">Connect your Instagram Creator account</p>
              <div class="method-features">
          ...
          <div class="connection-help">
            ...
            <p class="help-text">
              Make sure your Instagram account is set to Business or Creator mode. 
              You'll need admin access to connect the account.
            </p>
...
const accounts = ref([
  {
    ...
    recentPosts: [
      { id: 1, type: '📸', likes: 15400, comments: 342, date: '2h ago', color: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' },
      { id: 2, type: '🎬', likes: 28900, comments: 567, date: '1d ago', color: 'linear-gradient(135deg, #f093fb 0%, #f5576c 100%)' },
      { id: 3, type: '📷', likes: 12100, comments: 289, date: '2d ago', color: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)' }
    ]
  },
  {
    id: 'creative',
    ...
    recentPosts: [
      { id: 1, type: '🎨', likes: 22100, comments: 445, date: '5h ago', color: 'linear-gradient(135deg, #fa709a 0%, #fee140 100%)' },
      { id: 2, type: '📸', likes: 18700, comments: 321, date: '1d ago', color: 'linear-gradient(135deg, #a8edea 0%, #fed6e3 100%)' },
      { id: 3, type: '🎬', likes: 31200, comments: 678, date: '2d ago', color: 'linear-gradient(135deg, #d299c2 0%, #fef9d7 100%)' }
    ]
  },
  {
    id: 'personal',
    ...
    recentPosts: [
      { id: 1, type: '📸', likes: 3200, comments: 89, date: '1d ago', color: 'linear-gradient(135deg, #89f7fe 0%, #66a6ff 100%)' },
      { id: 2, type: '📷', likes: 2890, comments: 67, date: '3d ago', color: 'linear-gradient(135deg, #fdbb2d 0%, #22c1c3 100%)' },
      { id: 3, type: '🎬', likes: 4100, comments: 112, date: '5d ago', color: 'linear-gradient(135deg, #ff9a9e 0%, #fecfef 100%)' }
    ]
  }
])
...
const avgEngagement = computed(() => 
  accounts.value.reduce((sum, account) => sum + account.engagement, 0) / accounts.value.length
)
...
const filteredAccounts = computed(() => {
  ...
  if (selectedFilter.value !== 'all') {
    filtered = filtered.filter(account => account.status === selectedFilter.value)
  }
...
const drawModalChart = () => {
  ...
  const data = Array.from({ length: 30 }, (_, i) => 
    Math.sin(i * 0.2) * 1000 + 5000 + Math.random() * 500
  )
  ...
  data.forEach((value, index) => {
    ...
    if (index % 5 === 0) { // Only show every 5th point
      ctx.beginPath()
...
onMounted(async () => {
  ...
  accounts.value.forEach(account => {
    const canvas = document.querySelector(`[data-chart="${account.id}"]`) as HTMLCanvasElement
    if (canvas) {
      const data = Array.from({ length: 7 }, () => Math.random() * 1000 + 2000)
