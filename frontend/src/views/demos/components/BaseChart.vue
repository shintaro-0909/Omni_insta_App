...
<script setup lang="ts">
  ...
  const props = defineProps({
    ...
    type: {
      type: String as PropType<
        'line' | 'bar' | 'doughnut' | 'area' | 'scatter'
      >,
      default: 'line',
    },
    variant: {
    ...
    props.data.datasets.forEach((dataset, datasetIndex) => {
      const points = dataset.data.map((value, index) => ({
        ...
        y:
          padding +
          chartHeight -
          (value / Math.max(...dataset.data)) * chartHeight,
      }));
      ...
      if (props.gradientColors.length > 1) {
        ...
        props.gradientColors.forEach((color, index) => {
          gradient.addColorStop(
            index / (props.gradientColors.length - 1),
            color
          );
        });
      ...
      dataset.data.forEach((value, index) => {
        ...
        if (props.gradientColors.length > 1) {
          props.gradientColors.forEach((color, colorIndex) => {
            gradient.addColorStop(
              colorIndex / (props.gradientColors.length - 1),
              color
            );
          });
        } else {
          gradient.addColorStop(
            0,
            dataset.backgroundColor || themeColors.value.primary
          );
          gradient.addColorStop(
            1,
            dataset.backgroundColor || themeColors.value.secondary
          );
        }
  ...
  const drawDoughnutChart = () => {
    ...
    const total = props.data.datasets[0].data.reduce(
      (sum, value) => sum + value,
      0
    );
    ...
    props.data.datasets[0].data.forEach((value, index) => {
      ...
      const color =
        props.gradientColors[index % props.gradientColors.length] ||
