<template>
  <div class="schedule-gantt-view">
        ...
        <div class="account-filter">
          ...
          >
            <template #selection="{ item, index }">
              ...
              >
                <v-avatar left>
                  <v-img :src="item.raw.avatar" :alt="item.raw.username"></v-img>
                </v-avatar>
                {{ item.raw.username }}
              </v-chip>
    ...
    <div class="status-summary">
      <div class="summary-card" v-for="status in statusSummary" :key="status.type">
    ...
    <div class="gantt-container">
      <div class="gantt-sidebar">
        <div class="sidebar-header">
          ...
          >
            <v-icon>{{ allAccountsVisible ? 'mdi-eye-off' : 'mdi-eye' }}</v-icon>
          </v-btn>
        </div>
        ...
        <div class="account-rows">
          ...
          >
            ...
            <div class="account-actions">
              ...
              >
                <v-icon size="16">{{ account.visible ? 'mdi-eye' : 'mdi-eye-off' }}</v-icon>
              </v-btn>
          ...
          >
            ...
            >
              <div 
                ...
                :style="{ backgroundColor: account.color + '20', borderColor: account.color }"
                draggable="true"
                @dragstart="handleDragStart($event, schedule)"
                @click="openScheduleDetails(schedule)"
              >
                <div class="schedule-content">
                  <div class="schedule-time">{{ formatTime(schedule.scheduledAt) }}</div>
                  <div class="schedule-caption">{{ truncateText(schedule.caption, 30) }}</div>
    ...
    <div class="gantt-legend">
      ...
      <div class="legend-items">
        <div class="legend-item" v-for="status in legendStatuses" :key="status.type">
...
const weekStats = computed(() => ({
  ...
  completedPosts: schedules.value.filter(s => s.status === 'completed').length,
...
const legendTypes = [
  { type: 'once', icon: 'mdi-clock-outline', color: 'primary', label: '一回のみ' },
  { type: 'recurring', icon: 'mdi-repeat', color: 'success', label: '繰り返し' },
  { type: 'random', icon: 'mdi-shuffle', color: 'warning', label: 'ランダム' }
]

// Methods
function getSchedulesForCell(accountId: string, timeSlot: TimeSlot): Schedule[] {
...
function handleDrop(event: DragEvent, account: Account, timeSlot: TimeSlot) {
  ...
  if (event.dataTransfer) {
    const scheduleData = JSON.parse(event.dataTransfer.getData('application/json'))
...
.summary-icon.completed { background: #dcfce7; color: #16a34a; }
.summary-icon.failed { background: #fee2e2; color: #dc2626; }
.summary-icon.draft { background: #f1f5f9; color: #64748b; }
