        ...
        <v-col cols="12">
          <div class="d-flex align-center justify-space-between">
            <div class="d-flex align-center">
              <v-icon icon="mdi-server-network" size="32" color="primary" class="mr-3" />
              <div>
      ...
      <v-row>
        <v-col cols="12">
          <v-card class="proxies-list-card" elevation="0" rounded="xl">
            ...
            <!-- ローディング -->
            <div v-if="proxiesStore.loading && filteredProxies.length === 0" class="text-center pa-8">
              <v-progress-circular indeterminate color="primary" size="64" />
              <div class="mt-4 text-subtitle-1">プロキシを読み込み中...</div>
            </div>

            <!-- 空の状態 -->
            <div v-else-if="filteredProxies.length === 0" class="text-center pa-8">
              ...
              <div class="text-subtitle-1 text-medium-emphasis mb-4">
                {{ searchQuery ? '検索条件を変更してください' : '新しいプロキシを追加してください' }}
              </div>
              <v-btn
                v-if="!searchQuery"
                color="primary"
            ...
            <div v-else>
              <v-list>
                <template v-for="(proxy, index) in filteredProxies" :key="proxy.id">
...
const filteredProxies = computed(() => {
  ...
  if (protocolFilter.value) {
    filtered = filtered.filter(proxy => proxy.protocol === protocolFilter.value)
  }
