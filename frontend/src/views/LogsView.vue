        ...
        <v-col cols="12">
          <div class="d-flex align-center justify-space-between">
            <div class="d-flex align-center">
              <v-icon icon="mdi-chart-line" size="32" color="primary" class="mr-3" />
              <div>
              ...
              <div v-if="logsStore.stats" class="status-content">
                <div class="status-item">
                  <div class="d-flex align-center justify-space-between mb-2">
                    ...
                    <span class="status-value">{{ logsStore.stats.success }}</span>
                  </div>
                  <v-progress-linear
                ...
                <div class="status-item mt-4">
                  <div class="d-flex align-center justify-space-between mb-2">
                    ...
                    <span class="status-value">{{ logsStore.stats.failed }}</span>
                  </div>
                  <v-progress-linear
                ...
                <div class="status-item mt-4">
                  <div class="d-flex align-center justify-space-between mb-2">
                    <span class="status-label">
                      <v-icon icon="mdi-refresh" color="warning" size="16" class="mr-1" />
                      リトライ中
                    </span>
                    <span class="status-value">{{ logsStore.stats.retrying }}</span>
                  </div>
                  <v-progress-linear
                ...
                <div class="success-rate-display">
                  <div class="text-center">
                    <div class="success-rate-number">{{ logsStore.successRate }}%</div>
      ...
      <v-row>
        <v-col cols="12">
          <v-card class="logs-list-card" elevation="0" rounded="xl">
            ...
            <!-- ローディング -->
            <div v-if="logsStore.loading && filteredLogs.length === 0" class="text-center pa-8">
            ...
            <div v-else-if="filteredLogs.length === 0" class="text-center pa-8">
              ...
              <div class="text-subtitle-1 text-medium-emphasis mb-4">
                {{ hasFilters ? 'フィルター条件を変更してください' : 'まだ投稿が実行されていません' }}
              </div>
            </div>
                      ...
                      <div class="log-details">
                        <div class="d-flex align-center ga-4">
                          ...
                          <div class="log-metric">
                            ...
                            スケジュール: {{ log.scheduleId.substring(0, 8) }}...
                          </div>
                        </div>
                        ...
                        <div v-if="log.nextRetryAt" class="retry-info mt-2">
                          ...
                          >
                            次回リトライ: {{ logsStore.formatLogDate(log.nextRetryAt) }}
                          </v-chip>
                        </div>
                      </div>
                    </div>
    ...
    <v-dialog v-model="showDetailsDialog" max-width="600px">
      <v-card v-if="selectedLog">
        <v-card-title class="text-h5">
          <v-icon :icon="getStatusIcon(selectedLog.status)" :color="logsStore.getStatusColor(selectedLog.status)" class="mr-2" />
          ログ詳細
        </v-card-title>
        <v-divider />
        <v-card-text class="pa-6">
...
import { useLogsStore, type ExecutionLog, type LogFilters } from '@/stores/logs'
...
const hasFilters = computed(() => {
  return !!(searchQuery.value || statusFilter.value || timeFilter.value || accountFilter.value)
})
...
const exportLogs = () => {
  // CSV エクスポート機能
  const headers = ['実行日時', 'ステータス', 'アカウント', 'スケジュールID', 'エラー']
  const csvContent = [
    headers.join(','),
  ...
  link.setAttribute('download', `execution_logs_${new Date().toISOString().split('T')[0]}.csv`)
