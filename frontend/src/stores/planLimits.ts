...
export const usePlanLimitsStore = defineStore('planLimits', () => {
  ...
  const getUserLimitsAndUsageFn = httpsCallable(
    functions,
    'getUserLimitsAndUsage'
  );
  const checkInstagramAccountLimitFn = httpsCallable(
    functions,
    'checkInstagramAccountLimit'
  );
  const checkPostExecutionLimitFn = httpsCallable(
    functions,
    'checkPostExecutionLimit'
  );
  const checkScheduleCreationLimitFn = httpsCallable(
    functions,
    'checkScheduleCreationLimit'
  );
  const checkFeatureAvailabilityFn = httpsCallable(
    functions,
  ...
  const isUnlimited = computed(() => {
    ...
    return (
      ...
      limits.value.monthlyPostLimit === -1
    );
  });
  ...
  const canAddAccount = computed(() => {
    ...
    return (
      usage.value.instagramAccountCount < limits.value.instagramAccountLimit
    );
  });
