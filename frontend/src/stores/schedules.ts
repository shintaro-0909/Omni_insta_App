...
export const useSchedulesStore = defineStore('schedules', () => {
  ...
  const fetchSchedule = async (scheduleId: string, igAccountId: string): Promise<Schedule> => {
    try {
      loading.value = true
      error.value = null
  ...
  const updateSchedule = async (data: UpdateScheduleData): Promise<void> => {
    try {
      ...
      if (response.success) {
        ...
        if (index !== -1) {
          // 更新されたデータを取得
          const updatedSchedule = await fetchSchedule(data.scheduleId, data.igAccountId)
  ...
  const deleteSchedule = async (scheduleId: string, igAccountId: string): Promise<void> => {
    try {
      loading.value = true
      error.value = null
