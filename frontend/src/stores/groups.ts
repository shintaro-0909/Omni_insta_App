...
export const useGroupsStore = defineStore('groups', () => {
  ...
  const groupStats = computed(
    (): GroupStats => ({
      ...
      totalAccounts: groups.value.reduce(
        (total, group) => total + group.igAccountIds.length,
        0
      ),
    })
  );
  ...
  const getGroupById = computed(
    () => (id: string) => groups.value.find(group => group.id === id)
  );

  const getGroupsByProxyId = computed(
    () => (proxyId: string) =>
    ...
    try {
      ...
      if (result.data && (result.data as any).success) {
        ...
        const newGroup: Group = {
          ...data.group,
          createdAt: data.group.createdAt.toDate
            ? data.group.createdAt.toDate()
            : new Date(data.group.createdAt),
          updatedAt: data.group.updatedAt.toDate
            ? data.group.updatedAt.toDate()
