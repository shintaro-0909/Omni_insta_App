...
export const useProxiesStore = defineStore('proxies', () => {
  ...
  const proxyStats = computed(
    (): ProxyStats => ({
      ...
      avgResponseTime:
        proxies.value.length > 0
          ? proxies.value.reduce(
              (sum, proxy) => sum + (proxy.responseTime || 0),
              0
            ) / proxies.value.length
          : 0,
    })
  );

  const getProxyById = computed(
    () => (id: string) => proxies.value.find(proxy => proxy.id === id)
  );

  const getProxyGroupById = computed(
    () => (id: string) => proxyGroups.value.find(group => group.id === id)
  );
    ...
    try {
      ...
      if (result.data && (result.data as any).success) {
        ...
        proxies.value = data.proxies.map((proxy: any) => ({
          ...proxy,
          createdAt: proxy.createdAt.toDate
            ? proxy.createdAt.toDate()
            : new Date(proxy.createdAt),
          updatedAt: proxy.updatedAt.toDate
            ? proxy.updatedAt.toDate()
            : new Date(proxy.updatedAt),
          lastTestedAt: proxy.lastTestedAt
            ? proxy.lastTestedAt.toDate
              ? proxy.lastTestedAt.toDate()
              : new Date(proxy.lastTestedAt)
            : undefined,
        }));
  ...
  const createProxy = async (
    proxyData: Omit<Proxy, 'id' | 'createdAt' | 'updatedAt'>
  ) => {
    try {
      loading.value = true;
      error.value = null;
      ...
      if (result.data && (result.data as any).success) {
        ...
        const newProxy: Proxy = {
          ...data.proxy,
          createdAt: data.proxy.createdAt.toDate
            ? data.proxy.createdAt.toDate()
            : new Date(data.proxy.createdAt),
          updatedAt: data.proxy.updatedAt.toDate
            ? data.proxy.updatedAt.toDate()
            : new Date(data.proxy.updatedAt),
        };
      ...
      if (result.data && (result.data as any).success) {
        ...
        const updatedProxy: Proxy = {
          ...
          lastTestedAt: data.proxy.lastTestedAt
            ? data.proxy.lastTestedAt.toDate
              ? data.proxy.lastTestedAt.toDate()
              : new Date(data.proxy.lastTestedAt)
            : undefined,
        };
  ...
  const testProxy = async (proxyId: string) => {
    try {
      ...
      if (result.data && (result.data as any).success) {
        ...
        console.log(
          `✅ プロキシテスト完了: ${proxy.name} - ${isActive ? '成功' : '失敗'}`
        );
