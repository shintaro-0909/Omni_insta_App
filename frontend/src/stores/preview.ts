...
export const usePreviewStore = defineStore('preview', () => {
  ...
  const estimatedEngagement = computed(() => {
    ...
    const avgLikes = currentAccount.value.avgLikes || Math.floor(followers * 0.03)
    ...
    const estimatedLikes = Math.floor(avgLikes * (1 + hashtagBoost + timeBoost))
    ...
    return {
      ...
      engagementRate: ((estimatedLikes + estimatedComments) / followers * 100).toFixed(2)
    }
  })
  ...
  const setPreviewPost = (post: PreviewPost) => {
    ...
    // プレビュー履歴に追加
    if (post.id) {
      const existingIndex = previewHistory.value.findIndex(p => p.id === post.id)
      if (existingIndex >= 0) {
  ...
  const updateSettings = (newSettings: Partial<PreviewSettings>) => {
    ...
    // ローカルストレージに保存
    localStorage.setItem('instagram-preview-settings', JSON.stringify(settings.value))
  }
  ...
  const isPopularHashtag = (tag: string): boolean => {
    ...
    const popularTags = [
      ...
      'fashion', 'followme', 'nature', 'travel', 'art',
      'food', 'style', 'japan', 'tokyo', 'instagram'
    ]
  ...
  const getHashtagSuggestions = (currentTags: string[]): string[] => {
    ...
    const suggestions = [
      'instagood', 'photooftheday', 'instalike', 'instadaily',
      'followforfollow', 'likeforlike', 'instamood', 'bestoftheday'
    ]
