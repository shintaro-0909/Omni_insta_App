...
class FileImportanceManager {
   ...
   * 変更履歴を記録
   */
  private recordChange(fileName: string, level: 'low' | 'medium' | 'high', reason: string, approved: boolean): void {
  ...
  analyzeConsolidationImpact(sourceFiles: string[], targetFile: string): any {
    ...
    const maxLevel = impacts.reduce((max, current) => {
    ...
    return {
      ...
      recommendations: this.generateConsolidationRecommendations(impacts, maxLevel)
    }
  }
   ...
   * 統合推奨事項を生成
   */
  private generateConsolidationRecommendations(impacts: any[], level: 'low' | 'medium' | 'high'): string[] {
...
export function analyzeConsolidation(sourceFiles: string[], targetFile: string) {
  return fileImportanceManager.analyzeConsolidationImpact(sourceFiles, targetFile)
}
