...
// サポートしている言語の一覧
export const supportedLocales: { code: MessageLanguages, name: string, flag: string }[] = [
...
export const formatDate = (date: Date, locale?: MessageLanguages): string => {
  ...
  return new Intl.DateTimeFormat(currentLocale === 'ja' ? 'ja-JP' : 'en-US', options).format(date)
}

export const formatDateTime = (date: Date, locale?: MessageLanguages): string => {
  const currentLocale = locale || getCurrentLocale()
...
// 数値のフォーマット
export const formatNumber = (num: number, locale?: MessageLanguages): string => {
  const currentLocale = locale || getCurrentLocale()
  
  return new Intl.NumberFormat(currentLocale === 'ja' ? 'ja-JP' : 'en-US').format(num)
}

// 通貨のフォーマット
export const formatCurrency = (amount: number, currency = 'JPY', locale?: MessageLanguages): string => {
  const currentLocale = locale || getCurrentLocale()
...
// パーセンテージのフォーマット
export const formatPercentage = (value: number, locale?: MessageLanguages): string => {
  const currentLocale = locale || getCurrentLocale()
