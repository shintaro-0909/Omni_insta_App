      ...
      <div class="hero-content">
        ...
        <slot name="actions">
          <div class="actions-system" :class="{ revealed: actionsRevealed }">
            <button class="demo-btn primary" @click="$emit('primary-action')">
...
<script setup lang="ts">
  ...
  // Local reactive copy of metrics for hover states
  const localMetrics = ref(props.metrics.map(metric => ({ ...metric })));

  const onMetricHover = (index: number) => {
    const metric = localMetrics.value[index];
    if (metric) {
      metric.active = true;
    }
  };
  ...
  const initiateRevealSequence = async () => {
    ...
    }, props.revealDelay + 400);
    setTimeout(() => {
      actionsRevealed.value = true;
    }, props.revealDelay + 800);
    setTimeout(() => {
      previewRevealed.value = true;
    }, props.revealDelay + 1200);
  };
