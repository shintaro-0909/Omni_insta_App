...
import { useSchedulesStore, type CreateScheduleData, type UpdateScheduleData, type Schedule } from '@/stores/schedules'
...
const titleRules = [
  ...
  (v: string) => v.length <= 100 || 'スケジュール名は100文字以内で入力してください'
]
...
const maxIntervalRules = [
  ...intervalRules,
  (v: number) => v >= randomMinInterval.value || '最大間隔は最小間隔以上で入力してください'
]
...
const initializeForm = () => {
  if (props.schedule) {
    ...
    if (props.schedule.type === 'one_time' && props.schedule.nextRunAt) {
      ...
      oneTimeTime.value = date.toTimeString().split(' ')[0].substring(0, 5)
    }
...
const buildScheduleData = (): CreateScheduleData | UpdateScheduleData => {
  ...
  if (formData.value.type === 'one_time') {
    const scheduledTime = new Date(`${oneTimeDate.value}T${oneTimeTime.value}:00`)
...
const saveSchedule = async () => {
  ...
  try {
    ...
    if (isEdit.value) {
      ...
      scheduleId = await schedulesStore.createSchedule(scheduleData as CreateScheduleData)
    }
